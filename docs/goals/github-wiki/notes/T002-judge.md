...
|-------|--------|
| **task_id** | T003 |
| **objective** | Author GitHub-wiki source under `docs/wiki/` (Home, Install, Usage, GoalBuddy-Loop, Upstream-Parity, Troubleshooting) from README + `docs/PARITY.md`; fix Install `cd` to `GoalBuddy-Cursor-Port` in wiki and README; link README to wiki; add `scripts/publish-wiki.mjs` (or `docs/goals/github-wiki/notes/publish-wiki-operator.md`) documenting clone/push to `https://github.com/wstrege-kenosha/GoalBuddy-Cursor-Port.wiki.git` and web-UI fallback. Attempt publish if credentials exist; else stop with operator steps in receipt—no fake live-wiki claim. |
| **allowed_files** | `docs/wiki/**`, `README.md`, `scripts/publish-wiki.mjs`, `docs/goals/github-wiki/notes/**` |
| **verify** | See below |
...
- `docs/wiki/Install.md` — clone URL; `cd GoalBuddy-Cursor-Port`; install commands; no secrets
- `docs/wiki/Usage.md` — `/goal-prep`, `/goal`, verify, smoke goal pointer
- `docs/wiki/GoalBuddy-Loop.md` — Scout/Judge/Worker; `state.yaml`; board command
...
test -f docs/wiki/Home.md && test -f docs/wiki/Install.md && test -f docs/wiki/Usage.md && test -f docs/wiki/GoalBuddy-Loop.md && test -f docs/wiki/Upstream-Parity.md && test -f docs/wiki/Troubleshooting.md
rg -n "cd GoalBuddy-Cursor-Port" docs/wiki/Install.md README.md
rg -n "wiki" README.md
node scripts/publish-wiki.mjs --help
```

If `publish-wiki.mjs` is omitted, verify must include `test -f docs/goals/github-wiki/notes/publish-wiki-operator.md` with git/UI steps instead.
