...
function renderBoard(board) {
  ...
  document.title = board.goal.title ? board.goal.title + " - Cursor Curator Board" : "Cursor Curator Board";
...
function renderOracleStrip(board) {
  ...
  const signal = board.goal?.success_criteria?.signal || "";
  const finalProof = board.goal?.success_criteria?.final_proof || "";
...
function boardOptionLabel(board) {
  const title = board.title || board.slug || board.goalDir || "Cursor Curator board";
