...
**A Cursor operator can install from this repo, run Objective Prep on a test outcome, open the live board, and complete one Scout → Approval Gate → Worker cycle with receipts in `state.yaml`, verified by `node` scripts and doctor output captured in task receipts.**
