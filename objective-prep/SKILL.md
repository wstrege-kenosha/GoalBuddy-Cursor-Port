---
name: objective-prep
description: Cursor Curator Objective Prep for Cursor. Prepares docs/objectives/<slug>/ boards with objective.md, state.yaml, and notes/ for long-running work. Use when the user runs /objective-prep, wants an objective board, or needs structured intake before /objective.
...
- Create `docs/objectives/<slug>/objective.md`, `state.yaml`, `notes/.gitkeep`, `.cursor-curator-board/` as needed
...
7. Register this workspace for MCP (required once per project):
   ...
   ```bash
   node ~/.cursor/skills/cursor-curator/scripts/curator.mjs workspace register
   ```
...
node ~/.cursor/skills/cursor-curator/scripts/curator.mjs update
```
