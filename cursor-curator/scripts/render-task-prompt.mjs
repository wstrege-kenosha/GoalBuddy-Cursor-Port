  ...
  return {
    ...
    payload: {
      metadata: {
        ...
        goal_success_criteria: board.objective.success_criteria || null,
...
export function parseArgs(args) {
  ...
  if (!options.objectiveRoot && !options.boardPath) {
    throw new Error("Usage: curator prompt <goal-root> [--task T###] [--board path/to/state.yaml]");
  }
  return options;
}
...
export function loadBoard(boardPath) {
  ...
  if (!document || Number(document.version) !== 2) {
    throw new Error(`unsupported Cursor Curator state version in ${boardPath}`);
  }
...
function promptWarnings(board, task) {
  ...
  if (isWeakProof(board.objective.success_criteria?.signal)) {
  ...
  if (isWeakProof(board.objective.success_criteria?.final_proof)) {
  ...
  if (board.document?.rules?.intake_misfire_must_be_audited === true) {
    if (isWeakProof(board.objective.intake?.likely_misfire)) {
    ...
    if (isWeakProof(board.objective.intake?.interpreted_outcome)) {
