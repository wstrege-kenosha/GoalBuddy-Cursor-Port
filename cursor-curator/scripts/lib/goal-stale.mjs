...
export function discoverObjectiveStatePaths(roots = []) {
  const paths = new Set();
  for (const root of roots) {
    const goalsRoot = join(resolve(root), "docs", "objectives");
...
export function findBlockedTasksInText(text) {
  const tasks = [];
