...
export function parseReceiptFromText(text) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const receipt = parsed.cursor_curator_receipt_v1 ?? parsed;
      if (receipt && typeof receipt === "object" && receipt.task_id) {
        return { envelope: parsed.cursor_curator_receipt_v1 ? parsed : { cursor_curator_receipt_v1: receipt }, receipt };
      }
    } catch {
      /* try next */
    }
...
export function applyReceiptToState(statePath, receiptEnvelope, options = {}) {
  ...
  if (role === "worker" && receipt.result === "done") {
    const task = readWorkerTaskForVerify(text, taskId);
    verification = verifyWorkerReceiptForTask(task, receipt);
    text = replaceLastVerification(text, formatLastVerificationYaml(verification.last_verification));
  }
  ...
  if (receipt.result === "done") {
    if (nextActiveTask) {
      ...
      const completion = checkCompletionReadinessFromText(text, statePath);
      if (!completion.ready) {
        return {
          ok: false,
      ...
      text = replaceNestedScalar(text, "objective", "status", "done");
...
function checkCompletionReadinessFromText(text, statePath) {
  const tempPath = join(tmpdir(), `cursor-curator-completion-${randomBytes(8).toString("hex")}.yaml`);
  writeFileSync(tempPath, text, "utf8");
  try {
    return checkCompletionReadiness(tempPath);
  } finally {
    try {
      unlinkSync(tempPath);
    } catch {
...
function readWorkerTaskForVerify(text, taskId) {
  const block = taskBlock(text, taskId) || "";
  const verify = [];
  let inVerify = false;
  for (const line of block.split(/\r?\n/)) {
    if (/^\s{4}verify:\s*$/.test(line)) {
      inVerify = true;
      continue;
    }
    if (inVerify && /^\s{6}-\s+(.+?)\s*$/.test(line)) {
      verify.push(clean(line.replace(/^\s{6}-\s+/, "")));
      continue;
    }
    if (inVerify && /^\s{4}\S/.test(line)) break;
  }
  return { id: taskId, verify };
}

function replaceLastVerification(text, yamlFragment) {
  const lines = text.split(/\r?\n/);
  let checksStart = -1;
  let lastVerificationStart = -1;
  let lastVerificationEnd = -1;

  for (let index = 0; index < lines.length; index += 1) {
    if (/^checks:\s*$/.test(lines[index])) checksStart = index;
    if (/^\s{2}last_verification:\s*$/.test(lines[index])) {
      lastVerificationStart = index;
      lastVerificationEnd = index + 1;
      for (let cursor = index + 1; cursor < lines.length; cursor += 1) {
        if (/^\s{2}\S/.test(lines[cursor]) && !/^\s{2}last_verification:/.test(lines[cursor])) {
          lastVerificationEnd = cursor;
          break;
        }
        ...
        lastVerificationEnd = cursor + 1;
      }
      break;
    }
  }

  const fragmentLines = yamlFragment.split(/\r?\n/).slice(1);
  if (lastVerificationStart >= 0) {
    lines.splice(lastVerificationStart, lastVerificationEnd - lastVerificationStart, ...fragmentLines);
    return lines.join("\n");
  }

  if (checksStart >= 0) {
    lines.splice(checksStart + 1, 0, ...fragmentLines);
    return lines.join("\n");
  }
...
function validateGoalStateFromText(text, statePath) {
  const tempPath = join(tmpdir(), `cursor-curator-validate-${randomBytes(8).toString("hex")}.yaml`);
  writeFileSync(tempPath, text, "utf8");
  try {
