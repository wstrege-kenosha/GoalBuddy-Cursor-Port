...
export function resolveDbPath(workspaceRoot: string): string {
  return join(resolve(workspaceRoot), ".cursor-curator", "curator.db");
}
...
function runMigrations(db: Database): void {
  ...
  db.transaction(() => {
    ...
    if (currentMigrationVersion(db) < MIGRATION_VERSION) {
      db.query("INSERT INTO schema_migrations (version) VALUES (?)").run(MIGRATION_VERSION);
    }
  })();
}
...
export function openDatabase(
  ...
  const key = options.memory ? ":memory:" : resolve(workspaceRoot);
