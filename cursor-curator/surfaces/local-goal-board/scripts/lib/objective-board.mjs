...
const logoAssetPath = join(surfaceRoot, "assets", "curator-mark.png");
...
export function normalizeGoalBoard(document, goalDir = "<memory>") {
  if (!document || typeof document !== "object" || Array.isArray(document)) {
    throw new GoalBoardError("Objective state must be a YAML mapping.");
  }
  if (Number(document.version) !== 2) {
    throw new GoalBoardError("Only Cursor Curator v2 state.yaml files are supported.");
  }
  if (!document.objective || typeof document.objective !== "object") {
    throw new GoalBoardError("Missing objective metadata.");
  }
  ...
  if (activeTasks.length > 1) {
    throw new GoalBoardError("Objective state has more than one active task.");
  }
...
function attachTaskSubgoal(task, goalDir) {
  ...
  if (!existsSync(childStatePath)) {
    throw new GoalBoardError(`Missing sub-objective state for ${task.id}: ${task.subgoal.path}`);
  }
...
function validateChildSubgoalPath(task, goalDir, childStatePath) {
  if (task.subgoal.depth !== 1) {
    throw new GoalBoardError(`Invalid sub-objective depth for ${task.id}: only depth 1 is supported.`);
  }
  const childRelativePath = relative(goalDir, childStatePath);
  if (!isInsideRoot(childRelativePath)) {
    throw new GoalBoardError(`Invalid sub-objective path for ${task.id}: ${task.subgoal.path} must stay inside the objective root.`);
  }
  const parts = childRelativePath.split(/[\\/]+/);
  if (parts.length !== 3 || parts[0] !== "subgoals" || parts[2] !== "state.yaml") {
    throw new GoalBoardError(`Invalid sub-objective path for ${task.id}: ${task.subgoal.path} must be subgoals/<slug>/state.yaml.`);
  }
}
...
export function parseGoalStateText(text) {
  try {
    ...
    if (!lines.length) throw new GoalBoardError("Objective state is empty.");
