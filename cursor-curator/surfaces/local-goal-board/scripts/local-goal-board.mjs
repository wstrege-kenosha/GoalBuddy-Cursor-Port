...
import { createBoardPayload, writeBoardApp } from "./lib/objective-board.mjs";
import { BOARD_SKIN_IDS, DEFAULT_BOARD_SKIN } from "./lib/board-theme.mjs";
...
export async function main() {
  ...
  const goalDir = resolve(options.objective || "");
  if (!options.objective) throw new Error("Missing --goal docs/objectives/<slug>");
...
export function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--goal") {
      ...
      options.objective = arg.slice("--goal=".length);
    } else if (arg === "--host") {
...
async function registerWithBoardHub({ goalDir, host, port }) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Cursor Curator local board hub rejected ${goalDir}: ${message}`);
  }
...
function boardPathFor(goalDir, payload) {
  const slug = slugifyPathSegment(payload?.objective?.slug || basename(goalDir));
...
function sendUnregisteredBoardPath(response, pathname, boards, baseUrl) {
  ...
  response.end([
    ...
    "Start or rerun `node cursor-curator/scripts/curator.mjs board <goal-dir>` to register that goal on this same port, then open the printed /<slug>/ URL.",
    "",
    "Registered boards:",
...
function usage() {
  console.log(`Cursor Curator Local Goal Board

Usage:
  node cursor-curator/scripts/curator.mjs board docs/objectives/<slug>
  node cursor-curator/scripts/curator.mjs board docs/objectives/<slug> --once --json
