...
const smokeState = join(repoRoot, "docs/goals/sample-cursor-smoke/state.yaml");
...
test("validateGoalState warns on weak oracle", () => {
  const result = validateGoalState(join(fixturesDir, "weak-oracle/state.yaml"));
  assert.equal(result.ok, true);
  assert.ok(result.warnings.some((warning) => warning.includes("goal.oracle.signal")));
  assert.ok(result.warnings.some((warning) => warning.includes("goal.oracle.final_proof")));
});
...
test("agent warnings reference goalbuddy.mjs install not npx goalbuddy agents", async () => {
  ...
  writeFileSync(join(fixtureDir, "goal.md"), "# fixture\n");
...
`);
  ...
  assert.ok(scoutWarning.includes("goalbuddy.mjs install"));
  assert.ok(!scoutWarning.includes("npx goalbuddy agents"));
});
