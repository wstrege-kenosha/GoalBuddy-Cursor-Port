...
export function checkCompletionReadiness(statePath) {
  ...
  const goalStatus = nestedScalar(text, "goal", "status");
  const oracleSignal = pathScalar(text, ["goal", "oracle"], "signal");
  const oracleFinalProof = pathScalar(text, ["goal", "oracle"], "final_proof");
  const completionProof = pathScalar(text, ["goal", "intake"], "completion_proof");
  const oracleReady = !isWeakProof(oracleSignal) && !isWeakProof(oracleFinalProof) && !isWeakProof(completionProof);

  if (!oracleReady) {
    blockers.push("oracle is not concrete enough for completion (signal, final_proof, or completion_proof is weak).");
  }
  ...
  if (!auditReady) {
    blockers.push("missing final Judge/PM audit with decision complete and full_outcome_complete: true");
  }

  if (goalStatus === "done" && !validation.ok) {
    blockers.push("goal.status is done but state validation failed");
  }

  const ready = validation.ok && oracleReady && auditReady && unfinishedWorkers.length === 0 && activeTasks.length === 0;
