const RECEIPT_KEY = "goalbuddy_receipt_v1";
...
export function validateReceipt(input, options = {}) {
  ...
  if (!role) {
    errors.push("Could not infer receipt role; pass --role scout|judge|worker.");
  }
  ...
  if (role === "judge" && receipt.result === "done" && !receipt.decision) {
    errors.push("Judge receipt.decision is required when result is done.");
  }
...
function inferRole(receipt) {
  ...
  if (receipt.decision) return "judge";
...
function normalizeRole(value) {
  ...
  return ["scout", "judge", "worker"].includes(role) ? role : null;
}
