...
export function validateGoalState(statePath, options = {}) {
  ...
  function agentStatusWarning(agent, status) {
    ...
    if (status === "bundled_not_installed") {
      return `agents.${agent} is bundled_not_installed; /goal can continue through PM fallback, but dedicated ${agentLabel} delegation is unavailable until installed. Run: node goalbuddy/scripts/goalbuddy.mjs install — then restart Cursor.`;
    }
    if (status === "missing") {
      return `agents.${agent} is missing; /goal can continue through PM fallback, but dedicated ${agentLabel} delegation is unavailable. Run: node goalbuddy/scripts/goalbuddy.mjs install`;
    }
    return `agents.${agent} is unknown; /goal can continue through PM fallback, but dedicated ${agentLabel} delegation was not verified. Run: node goalbuddy/scripts/goalbuddy.mjs doctor`;
  }
