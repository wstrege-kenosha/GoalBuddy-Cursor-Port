# goal-board

Start or refresh the GoalBuddy local board for a goal in this workspace.

## Steps

1. Determine the goal directory:
   - If the user named `docs/goals/<slug>`, use that path.
   - Else find the most recently modified `docs/goals/*/state.yaml` in the workspace, or ask which slug.

2. Run the board server (background if long-running):

   ```bash
   node ~/.cursor/skills/goalbuddy/scripts/goalbuddy.mjs board docs/goals/<slug>
   ```

3. Tell the user the board URL as a clickable Markdown link:

   `[Open GoalBuddy board](http://goalbuddy.localhost:41737/<slug>/)`

4. If port 41737 is in use, follow script output — check `http://127.0.0.1:41737/api/boards` before killing processes.

## Reference

`~/.cursor/skills/goalbuddy/surfaces/local-goal-board/README.md`
